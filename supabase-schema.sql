...
create policy "users add comments" on public.listing_comments for insert to authenticated with check (author_id = auth.uid() and exists (select 1 from public.listings where listings.id = listing_comments.listing_id and listings.status = 'active'));
