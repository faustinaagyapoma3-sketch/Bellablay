...
const CATEGORY_TONES = { Fashion: 'fashion', Electronics: 'electronics', 'Mobile Phones & Tablets': 'electronics', Property: 'home', 'Home, Furniture & Appliances': 'home', Vehicles: 'vehicles', Furniture: 'furniture', Services: 'services', 'Beauty & Personal Care': 'fashion', 'Repair & Construction': 'services', 'Commercial Equipment': 'electronics', 'Leisure & Activities': 'other', 'Babies & Kids': 'fashion', 'Food, Agriculture & Farming': 'home', 'Animals & Pets': 'other', Jobs: 'services', 'Seeking Work - CVs': 'services', Other: 'other' };
...
const normaliseWhatsApp = (value = '') => { const digits = String(value).replace(/\D/g, ''); if (digits.length === 10 && digits.startsWith('0')) return `233${digits.slice(1)}`; if (digits.length === 9) return `233${digits}`; return digits; };
const formatGhs = (value = '') => { const text = String(value).trim(); if (!text || /price on request/i.test(text)) return 'Price on request'; if (/^ghs\s*/i.test(text)) return `GHS ${text.replace(/^ghs\s*/i, '')}`; if (/^gh₵\s*/i.test(text)) return `GHS ${text.replace(/^gh₵\s*/i, '')}`; return `GHS ${text}`; };
...
function activeListings() {
  return listings.filter((listing) => {
    ...
    const searchable = `${listing.title} ${listing.description} ${listing.specifications} ${listing.category} ${listingSeller(listing)}`.toLowerCase();
...
function renderFeed() {
  ...
  const searchLabel = searchQuery || locationQuery ? ' · filtered results' : ' · newest first';
  byId('feed-count').textContent = `${visible.length} active listing${visible.length === 1 ? '' : 's'}${searchLabel}`;
  byId('listing-feed').innerHTML = visible.map((listing) => `<article class="listing-card">${listingVisual(listing)}<div class="listing-copy"><div><p class="seller-name">By ${escapeHtml(listingSeller(listing))}</p><h3>${escapeHtml(listing.title)}</h3><p class="listing-location">${escapeHtml(listing.location)} · ${formatAge(listing.created_at)}</p></div><strong class="listing-price">${escapeHtml(formatGhs(listing.price))}</strong></div><div class="card-footer"><span class="comment-count">${listingComments(listing).length} comment${listingComments(listing).length === 1 ? '' : 's'}</span><button data-action="view" data-id="${listing.id}">View & comments <span>→</span></button></div></article>`).join('') || '<div class="empty-feed"><h3>No listings in this category yet.</h3><p>Be the first person to post something useful.</p></div>';
...
function setAuthMode(login) {
  isLoginMode = login;
  byId('auth-title').textContent = login ? 'Sign in' : 'Create your account';
  byId('auth-copy').textContent = login ? 'Use your Gmail address and password to manage listings, messages, and account activity.' : 'Choose a public name once, then use your Gmail address and password to sign in later.';
  byId('name-field').hidden = login;
  byId('name-field').querySelector('input').required = !login;
  byId('password-input').autocomplete = login ? 'current-password' : 'new-password';
  byId('password-input').placeholder = login ? 'Your password' : 'Create a password (at least 8 characters)';
...
function openDetail(id) {
  ...
  const allComments = listingComments(listing);
  const comments = allComments.filter((comment) => !comment.parent_comment_id).map((comment) => {
    const replies = allComments.filter((reply) => reply.parent_comment_id === comment.id).map((reply) => `<div class="comment seller-reply"><b>${escapeHtml(reply.profiles?.display_name || 'Seller')} <span>Seller reply</span></b><small>${formatAge(reply.created_at)}</small><p>${escapeHtml(reply.body)}</p></div>`).join('');
    const replyButton = isSeller ? `<button class="reply-button" data-action="reply" data-comment="${comment.id}" data-name="${escapeHtml(comment.profiles?.display_name || 'customer')}">Reply as seller</button>` : '';
    return `<div class="comment"><b>${escapeHtml(comment.profiles?.display_name || 'Member')} · ${'★'.repeat(comment.rating)}</b><small>${formatAge(comment.created_at)}</small><p>${escapeHtml(comment.body)}</p>${replyButton}${replies ? `<div class="reply-thread">${replies}</div>` : ''}</div>`;
  }).join('') || '<p class="no-comments">No customer reviews yet.</p>';
  const whatsappNumber = normaliseWhatsApp(listing.whatsapp);
  const contactCard = listing.phone || whatsappNumber ? `<div class="seller-contact"><b>Contact this seller</b><div class="contact-actions">${listing.phone ? `<a class="phone-link" href="tel:${normaliseWhatsApp(listing.phone)}"><span>☎</span>${escapeHtml(listing.phone)}</a>` : ''}${whatsappNumber ? `<a class="whatsapp-link" href="https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Hello, I am interested in your ${listing.title} on BellaBlay Market.`)}" target="_blank" rel="noopener noreferrer"><span class="whatsapp-mark">◔</span>WhatsApp seller</a>` : ''}</div></div>` : '';
  const discussionForm = isSeller ? `${replyingToComment ? `<form class="seller-reply-form" id="seller-reply-form"><p>Replying to <b>${escapeHtml(replyingToComment.name)}</b></p><input type="hidden" name="parentCommentId" value="${replyingToComment.id}" /><textarea name="reply" required maxlength="220" placeholder="Write a helpful reply to this customer."></textarea><button type="submit">Send seller reply</button></form>` : '<p class="seller-discussion-note">Only other customers can leave a review on your item. Select <b>Reply as seller</b> below a customer comment to respond.</p>'}` : `<form class="review-form" id="review-form"><select name="rating" aria-label="Rating"><option value="5">5★</option><option value="4">4★</option><option value="3">3★</option><option value="2">2★</option><option value="1">1★</option></select><input name="comment" required maxlength="220" placeholder="Write a respectful question or review" /><button type="submit">Post</button></form>`;
  byId('detail-content').innerHTML = `<div class="detail-layout"><div class="detail-media" data-tone="${tone}">${photos[0] ? `<img src="${photos[0].public_url}" alt="${escapeHtml(listing.title)}" />` : '<span class="visual-shape"></span><span class="visual-shadow"></span>'}${video ? `<a class="video-link" href="${video.public_url}" target="_blank" rel="noreferrer">Watch listing video ↗</a>` : ''}</div><div class="detail-copy"><p class="seller-name">Posted by ${escapeHtml(listingSeller(listing))} · ${formatAge(listing.created_at)}</p><h2>${escapeHtml(listing.title)}</h2><strong class="detail-price">${escapeHtml(formatGhs(listing.price))}</strong><p>${escapeHtml(listing.description)}</p><div class="specs"><b>Specifications</b><br />${escapeHtml(listing.specifications)}</div>${contactCard}${!isSeller ? `<button class="button dark full" data-action="message" data-id="${listing.id}">${user ? 'Message seller' : 'Sign in to message seller'} <span>→</span></button>` : '<p class="dashboard-note">This is your listing. Interested buyers appear in your account dashboard.</p>'}<div class="comment-section"><h3>Customer comments & reviews</h3><div id="comments-list">${comments}</div>${discussionForm}</div></div></div>`;
...
async function submitListing(event) {
  ...
  if (!selectedMedia.photos.length) { setMessage('listing-message', 'Add at least one clear photo of the item before publishing.', true); return; }
  setMessage('listing-message', 'Publishing your listing…');
  const { data: listing, error } = await supabase.from('listings').insert({ seller_id: user.id, title: data.get('title').trim(), category: data.get('category'), price: formatGhs(data.get('price')), location: data.get('location').trim(), description: data.get('description').trim(), specifications: data.get('specifications').trim(), phone: data.get('phone').trim() || null, whatsapp: data.get('whatsapp').trim() || null, status: 'draft' }).select().single();
  if (error) throw error;
  ...
  const { error: publishError } = await supabase.from('listings').update({ status: 'active' }).eq('id', listing.id);
...
async function submitSellerReply(event) {
  event.preventDefault();
  if (!selectedListing || user?.id !== selectedListing.seller_id) return;
  const data = new FormData(event.target);
  const { error } = await supabase.from('listing_comments').insert({ listing_id: selectedListing.id, author_id: user.id, parent_comment_id: data.get('parentCommentId'), rating: 5, body: data.get('reply').trim() });
  if (error) throw error;
...
function handleClick(event) {
  ...
  if (action?.dataset.action === 'reply') { replyingToComment = { id: action.dataset.comment, name: action.dataset.name }; openDetail(selectedListing.id); }
...
byId('rail-post').addEventListener('click', openListingForm);
byId('mobile-post').addEventListener('click', openListingForm);
byId('mobile-dashboard').addEventListener('click', () => openDashboard().catch(showError));
byId('mobile-profile').addEventListener('click', () => { if (user) openDashboard().catch(showError); else { setAuthMode(true); openModal('account-modal'); } });
byId('auth-toggle').addEventListener('click', () => setAuthMode(!isLoginMode));
const chooseCategory = (category) => { activeCategory = category; byId('search-category').value = category; renderFeed(); };
byId('category-filters').addEventListener('click', (event) => { const button = event.target.closest('button[data-category]'); if (!button) return; chooseCategory(button.dataset.category); });
byId('category-card-grid').addEventListener('click', (event) => { const button = event.target.closest('button[data-category]'); if (!button) return; if (button.classList.contains('post-tile')) { openListingForm(); return; } chooseCategory(button.dataset.category); document.getElementById('listing-feed').scrollIntoView({ behavior: 'smooth', block: 'start' }); });
byId('search-form').addEventListener('submit', (event) => { event.preventDefault(); searchQuery = byId('search-input').value.trim().toLowerCase(); locationQuery = byId('location-filter').value.trim().toLowerCase(); activeCategory = byId('search-category').value; renderFeed(); document.getElementById('browse').scrollIntoView({ behavior: 'smooth', block: 'start' }); });
...
byId('detail-content').addEventListener('submit', (event) => { if (event.target.id === 'review-form') submitComment(event).catch(showError); if (event.target.id === 'seller-reply-form') submitSellerReply(event).catch(showError); });
...
document.querySelectorAll('[data-chat-prompt]').forEach((button) => button.addEventListener('click', () => { byId('chat-form').querySelector('textarea[name="message"]').value = button.dataset.chatPrompt; byId('chat-form').querySelector('textarea[name="message"]').focus(); }));
